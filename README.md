...
- 游戏默认开启声音，可在探索地图右上角随时静音；学习卡和提示使用系统语音朗读，四款放松游戏播放离线操作与反馈音效。
- Web代码、学习图片和游戏资源全部随安装包提供，Android应用不申请联网权限，也不依赖服务器、CDN或首次下载。
- Android 和 iOS 优先使用原生系统语音，Web 和 Electron 使用浏览器系统语音作为回退；Android 平板需在系统的文字转语音设置中安装中文离线语音数据。
- Capacitor日志中的 `https://localhost` 是应用进程内的本地资源地址，不是互联网服务器；系统没有可用的中文语音时，应用会保留游戏音效并在开启声音时提示语音不可用。
...
桌面端由 Electron 打包，移动端由 Capacitor 复用同一份 React 和 Phaser 游戏代码。iPhone、iPad、Android 手机和 Android 平板支持竖屏与横屏旋转，并会避开刘海和系统安全区；竖屏保持 16:9 游戏画面比例，将虚拟控制放在画布下方。
...
- 手柄：方向键选择，A 确认，B 返回；地图中 Y 打开放松站，L1 打开学习馆，R1 打开伙伴册。
- 移动端：四款放松游戏显示半透明动态控制。雷光飞行使用左摇杆，其余游戏使用所需方向键；右侧只显示当前可用的 A/B 操作。
- 雷光飞行：鼠标或手指拖动飞船；方向键、虚拟摇杆或实体手柄摇杆可做二维移动；空格、J 或 A 使用雷暴。
- 打地鼠：点击或轻触洞口；方向键或手柄选择洞位，Enter 或 A 敲击。
- 彩虹方块：左右移动，向上或 A 旋转，向下快速落下；旋转、落地和消除行使用不同反馈音效。
- 星芽拉力赛：轻触赛道直接选车道；`A`/`D`、方向键或手柄切换车道，空格、Enter 或 A 使用氮气。
